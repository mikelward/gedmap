...
- **The lockfile check asks each CONSUMER what it resolves; it does not match
  ...
  **The root and any workspaces are seeded directly**, because they are where
  ...
  with half its rules gone. `scripts/check-npm-update.test.js` covers
  ...
  **The tree check looks at IGNORED paths too, in a second `git status` call.**
  ...
  `npm-update.test.js` pins both the second call and the
  ...
  - **`npm-update.test.js` asserts those checks stay in step with
