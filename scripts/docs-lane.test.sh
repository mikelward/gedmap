...
check "markdown-only diff is docs"        0 "docs_only=true"  FILES="README.md spec.md"
check "code file makes it code"           0 "docs_only=false" FILES="README.md src/App.jsx"
check "markdown under src/ is a build input" 0 "docs_only=false" FILES="src/notes.md"
check "markdown under public/ ships verbatim" 0 "docs_only=false" FILES="public/help.md"
...
check "code renamed into docs is code"    0 "docs_only=false" FILES="A.md:src/a.ts"
...
ALLOK="ci=success"
ALLSKIP="ci=skipped"
check "all heavy jobs green passes"       0 "" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK"
check "justified skip with prefixes"      0 "" FILES="README.md" SUBJECTS="docs: Fix a typo" CLASSIFY=success RESULTS="$ALLSKIP"
check "deps prefix fails the lane"        1 "lacks a housekeeping prefix" FILES="TODO.md" SUBJECTS="deps: Update dependencies (2026-08-16)" CLASSIFY=success RESULTS="$ALLSKIP"
check "todo prefix rides the lane"        0 "" FILES="TODO.md" SUBJECTS="todo: Check off the lane rollout" CLASSIFY=success RESULTS="$ALLSKIP"
check "build prefix fails the lane"       1 "lacks a housekeeping prefix" FILES="README.md" SUBJECTS="build: Pin the runner major" CLASSIFY=success RESULTS="$ALLSKIP"
...
check "skip on a code diff is refused"    1 "refusing the skip" FILES="src/a.ts" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP"
...
check "green results from an outlived run are refused" 1 "own run owns the verdict" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" HEAD_SHA=otherhead
check "green results from a retargeted run are refused" 1 "own run owns the verdict" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" BASE_REF=other
check "green results on a shared head are refused" 1 "cannot vouch for exactly one" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" SHARED_PRS="1 2"
check "a push run's green gate reports"   0 "" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" GITHUB_EVENT_NAME=push
check "a dispatched green gate settles too"  1 "moved while the gate was reading" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" GITHUB_EVENT_NAME=workflow_dispatch BASE_AFTER=2
check "a dispatched green gate reports when settled" 0 "" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" GITHUB_EVENT_NAME=workflow_dispatch
check "green results after a late retarget are refused" 1 "moved while the gate was reading" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" BASE_AFTER=2
check "green results after a late force-push are refused" 1 "moved while the gate was reading" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" HEAD_AFTER=2
check "green results after a late twin PR are refused" 1 "moved while the gate was reading" FILES="src/a.ts" CLASSIFY=success RESULTS="$ALLOK" PULLS_AFTER=2
...
check "red job fails the gate"            1 "not all green" FILES="src/a.ts" CLASSIFY=success RESULTS="ci=failure"
# GitHub spells an interrupted job's result value the British way; this
# repo's US-spelling scan has no exemption list, so the fixture uses a
# stand-in — the engine's wildcard arm treats every value that is not
# success or skipped alike, so the arm under test is the same one.
check "an interrupted job fails the gate" 1 "not all green" FILES="src/a.ts" CLASSIFY=success RESULTS="ci=interrupted"
check "failed classify fails the gate"    1 "nothing vouches" FILES="src/a.ts" CLASSIFY=failure RESULTS="$ALLSKIP"
